...
```{r, message=TRUE}
message("**Repository** can benefit form the qualities of the [**Singleton** pattern](./articles/Singleton.html). This is why Using `inherit = Singleton` the **Repository** is ")
```
...
Repository implementation are project specific. The following implementations
consider a **Repository** of car models with their specifications.

From the caller perspective, both implementation behave identically -- they have
the same queries. Nevertheless, Under the hood the two implementations employ
...
First we define `initialize` (class constructor). When the caller instantiate a new `TransientRepository` object, 
