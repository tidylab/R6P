...
```{r, message=TRUE}
message("By passing the input argument `inherit = Singleton`, the `AbstractRepository` inherits the qualities of the [**Singleton** pattern](./Singleton.html).")
```
...
implementation consider a **Repository** of car models with their
specifications.
...
In the example, we use the `mtcars` dataset with a `uid` column that uniquely
