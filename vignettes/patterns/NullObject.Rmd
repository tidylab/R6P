...
```{r child=file.path(usethis::proj_get(), "vignettes", "details", "_NullObject.Rmd")}
```
...
-   Useful as part of a [gracefully
    failing](https://en.wikipedia.org/wiki/Graceful_exit) strategy.
...
-   Useful in shiny dashboards when a visual is dependent on the user selection
    of what to plot. In this case, you could also add a "call to action" text as
    a ggplot object.
